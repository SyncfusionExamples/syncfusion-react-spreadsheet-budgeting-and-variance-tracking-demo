...
import './App.css';
import { budgetData, actualDataSource } from './data-util';
import {
...
export default function App() {
    ...
    const DashboardInsights = (spreadsheet) => {
        ...
        setCell(41, 7, dashboardSheet, { value: 'TOP 5 DELIQUENT BRANCHES', colSpan: 3, style: { fontFamily: 'Times New Roman', fontSize: '11pt', verticalAlign: 'middle', textAlign: 'center' } });
        setCell(41, 2, dashboardSheet, { value: 'LOW PERFORMING BRANCHES', colSpan: 2, style: { fontFamily: 'Times New Roman', fontSize: '11pt', verticalAlign: 'middle', textAlign: 'center' } });
        ...
        deliquentHeaders.forEach((val, i) => {
            setCell(42, i + 7, dashboardSheet, { value: val, style: { color: '#124B5C', fontWeight: 'bold', verticalAlign: 'middle' } });
        });
        branchSpend.forEach((val, i) => {
            setCell(42, i + 1, dashboardSheet, { value: val, style: { color: '#124B5C', fontWeight: 'bold', verticalAlign: 'middle' } });
        });
    ...
    const chartDataPopulation = (spreadsheet) => {
        ...
        locations.forEach((val) => {
            setCell(branchData, 0, sheet, { value: val });
            setCell(branchData, 5, sheet, { value: val });
            setCell(paymentRiskData, 0, sheet, { value: val });
            setCell(paymentRiskData, 6, sheet, { value: val });
            setCell(paymentRiskData, 9, sheet, { value: val });
            setCell(paymentRiskData, 11, sheet, { value: val });
            setCell(paymentRiskData, 16, sheet, { value: val });
            setCell(paymentRiskData, 12, sheet, { formula: `=COUNTIFS(Actuals!D4:D1004,Dashboard!L${paymentRiskData + 1},Actuals!L4:L1004,"Full")` });
            setCell(paymentRiskData, 13, sheet, { formula: `=COUNTIFS(Actuals!D4:D1004,Dashboard!L${paymentRiskData + 1},Actuals!L4:L1004,"Partial")` });
            setCell(paymentRiskData, 14, sheet, { formula: `=COUNTIFS(Actuals!D4:D1004,Dashboard!L${paymentRiskData + 1},Actuals!L4:L1004,"Missed")` });
            setCell(paymentRiskData, 1, sheet, { formula: `=AVERAGEIF(Variance!C8:C37,A${paymentRiskData + 1},Variance!K8:K37)` });
            setCell(paymentRiskData, 17, sheet, { formula: `=COUNTIFS(Actuals!D4:D1004,A${paymentRiskData + 1},Actuals!P4:P1004,"Yes")` });
            setCell(paymentRiskData, 18, sheet, { formula: `=ROUND(AVERAGEIFS(Actuals!N4:N1004,Actuals!D4:D1004,A${paymentRiskData + 1},Actuals!P4:P1004,"Yes"),0)` });
            setCell(paymentRiskData, 10, sheet, { formula: `=AVERAGEIF(Variance!C8:C37,Dashboard!J${paymentRiskData + 1},Variance!K8:K37)` });
            setCell(paymentRiskData, 7, sheet, { formula: `=SUMIF(Variance!C8:C37,Dashboard!A${paymentRiskData + 1},Variance!F8:F37)` });
            paymentRiskData++;
            branchData++;
        });
        for (let k = 9; k < 19; k++) {
    ...
    const varianceSheetCalculation = (spreadsheet) => {
        ...
        const performanceHeader = ['Expected Spend', 'Actual Spend', 'Variance (Actual - Expected)', 'Variance% (vs Expected)'];
        ...
        setCell(6, 11, sheet, { style: { fontWeight: 'bold', verticalAlign: 'middle', textAlign: 'center', fontSize: '14pt', backgroundColor: '#124B5C', color: '#fff' }, value: 'Risk Level' });
        ...
        for (let i = 4; i <= 7; i++) {
            if (i == 6 || i == 7) {
                setCell(6, i, sheet, { style: { fontWeight: 'bold', verticalAlign: 'middle', textAlign: 'center', fontSize: '11pt', backgroundColor: '#1E6B2D', color: '#fff' }, value: performanceHeader[i - 4], wrap: true });
            }
            else {
        ...
        for (let i = 7; i < 37; i++) {
            ...
            setCell(i, 5, sheet, { formula: `=SUMIFS(actuals!H4:H1000, actuals!D4:D1000, Variance!C${i + 1}, actuals!E4:E1000, Variance!D${i + 1})`, style:varianceSheetColumnStyle });
            ...
            setCell(i, 9, sheet, { formula: `=SUMIFS(actuals!O4:O1000, actuals!D4:D1000, Variance!C${i + 1}, actuals!E4:E1000, Variance!D${i + 1})`, style:varianceSheetColumnStyle  });
            setCell(i, 10, sheet, {
        ...
        //number format, conditional format, setborder
